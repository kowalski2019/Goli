<template>
  <Modal :show="true" title="Edit Pipeline" size="lg" @close="$emit('close')">
...
import { updatePipeline, getPipeline } from '../api/client'
...
async function handleSubmit() {
  ...
  try {
    // Prepare variables object - include all variables (masked ones will be preserved by backend)
    const variables = {}
    for (const variable of formData.value.variables) {
      if (variable.name) {
        // Include all variables - backend will preserve masked secrets
    ...
    error.value = e.message || 'Error updating pipeline'
  } finally {
    loading.value = false
  }
}

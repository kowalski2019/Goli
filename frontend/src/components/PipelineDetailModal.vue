  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="$emit('close')">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
      ...
      <div class="p-6 space-y-6">
        <!-- Pipeline Info -->
        <div>
          <h4 class="text-sm font-medium text-gray-500 mb-2">Pipeline Information</h4>
          <dl class="grid grid-cols-2 gap-4">
            <div>
              <dt class="text-sm text-gray-500">ID</dt>
              <dd class="mt-1 text-sm text-gray-900">#{{ pipeline.id }}</dd>
            </div>
            <div>
              <dt class="text-sm text-gray-500">Name</dt>
              <dd class="mt-1 text-sm text-gray-900">{{ pipeline.name }}</dd>
            </div>
            <div>
              <dt class="text-sm text-gray-500">Description</dt>
              <dd class="mt-1 text-sm text-gray-900">{{ pipeline.description || '-' }}</dd>
            </div>
            <div>
              <dt class="text-sm text-gray-500">Created At</dt>
              <dd class="mt-1 text-sm text-gray-900">{{ formatDate(pipeline.created_at) }}</dd>
            </div>
          </dl>
        </div>
        ...
        <!-- Pipeline Definition -->
        <div>
          <h4 class="text-sm font-medium text-gray-500 mb-2">Pipeline Definition</h4>
          <div class="bg-gray-900 rounded-lg p-4 overflow-x-auto">
            <pre class="text-sm text-green-400 font-mono whitespace-pre-wrap">{{ pipeline.definition }}</pre>
          </div>
        </div>

        <!-- Recent Jobs -->
        <div v-if="recentJobs.length > 0">
          <h4 class="text-sm font-medium text-gray-500 mb-2">Recent Jobs</h4>
          <div class="space-y-2">
            <div
            ...
            >
              <div class="flex items-center justify-between">
                <div>
                  <div class="font-medium text-gray-900">#{{ job.id }} - {{ job.name }}</div>
